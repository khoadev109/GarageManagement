...
export class AccessaryUnitPagingMapping implements ReduxMapping {

    mapStateToProps(state: any, ownProps?: any) : PagingProps {
        return {
            deleteResult: state.AccessaryUnitDeleteReducer,
            pagingResult: state.AccessaryUnitWithPagingReducer     
        }
    }
    
    mapDispatchToProps(dispatch: any, ownProps?: any) : PagingDispatchProps {
        const pagingAction = new FetchAction.AccessaryUnitPagingAction();
        const deleteAction = new PostAction.AccessaryUnitDeleteAction();
