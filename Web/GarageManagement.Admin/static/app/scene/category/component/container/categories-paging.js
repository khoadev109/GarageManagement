...
class Categories extends React.Component {
    constructor(props) {
        ...
        this.pagingNavigation = new PagingNavigation.NavigationSetting({
            NavigationId: "Category_Navigation",
    ...
    showUpdateModal(categoryId) {
        this.setState({
            Title: "Cập nhật danh mục",
            SelectedIdOnRow: categoryId,
            IsOpenCreateOrUpdateModal: true
        });
    }
    showConfirmDeleteModal(categoryId) {
        this.setState({
            Title: "Xóa danh mục",
            SelectedIdOnRow: categoryId,
    ...
    onLoadCategories() {
        this.props.loadPagingCategories(this.state.Request);
    }
    ...
    showItemsPerPage() {
        return (React.createElement(ItemsPerPage, { Id: "Category_ItemsPerPage", SelectedValue: this.state.Request.PageSize, onSelectItemsPerPage: (numberSelected) => this.onSelectItemsPerPage(numberSelected) }));
    }
    onLoadCategoriesByNextNavigation() {
        this.state.Request.PageIndex += 1;
        this.isSelectItemsPerPage = false;
        this.onLoadCategories();
        this.pagingNavigation.setGroupPageIndexForNextNavigation(this.state);
    }
    onLoadCategoriesByPrevNavigation() {
        this.state.Request.PageIndex -= 1;
        this.isSelectItemsPerPage = false;
        this.onLoadCategories();
        this.pagingNavigation.setGroupPageIndexForPrevNavigation(this.state);
    }
    onLoadCategoriesByPageSelected(pageIndex) {
    ...
    showNavigationButtons() {
        ...
        if (pagingResult.Success) {
            return this.pagingNavigation.showNavigationButtons(this.state, this.isSelectItemsPerPage, () => this.onLoadCategoriesByNextNavigation(), () => this.onLoadCategoriesByPrevNavigation(), (pageIndex) => this.onLoadCategoriesByPageSelected(pageIndex));
        }
    }
    ...
    fetchCategories() {
        ...
        if (pagingResult.Success) {
            ...
            return (dtosResponse.length ?
                dtosResponse.map((category, i) => { return this.fetchCategory(category); }) : React.createElement(NoData, null));
        }
    }
    fetchCategory(item) {
        return (React.createElement("tr", { className: "gradeA odd", role: "row", key: item.Id },
            React.createElement("td", null, item.Name),
            React.createElement("td", null, item.ParentName),
    ...
    render() {
        return (React.createElement(React.Fragment, null,
                ...
                React.createElement("div", { className: "col-lg-12" },
                    React.createElement("div", { className: "ibox float-e-margins" },
                        React.createElement("div", { className: "ibox-title" },
                            React.createElement("h5", null, "Danh s\u00E1ch danh m\u1EE5c")),
                            ...
                            React.createElement("div", { className: "table-responsive" },
                                React.createElement("div", { id: "DataTables_Table_0_wrapper", className: "dataTables_wrapper form-inline dt-bootstrap" },
                                    ...
                                    React.createElement("div", { id: "DataTables_Table_0_filter", style: { float: "right" }, className: "dataTables_filter" },
                                        ...
                                        React.createElement("button", { type: "button", className: "btn btn-w-m btn-primary", onClick: () => this.onLoadCategories() }, "T\u00ECm")),
                                    ...
                                    React.createElement("table", { id: "DataTables_Table_0", "aria-describedby": "DataTables_Table_0_info", role: "grid", className: "table table-striped table-bordered table-hover dataTables-example dataTable" },
                                        React.createElement("thead", null,
                                            React.createElement("tr", { role: "row" },
                                                React.createElement("th", { className: this.showSortStatus("Name"), onClick: () => this.onSorting("Name") }, "T\u00EAn danh m\u1EE5c"),
                                                React.createElement("th", { className: this.showSortStatus("ParentName"), onClick: () => this.onSorting("ParentName") }, "Danh m\u1EE5c cha"),
                                                React.createElement("th", { colSpan: 2 }))),
                                        React.createElement("tbody", null, this.fetchCategories())),
            ...
            React.createElement(GeneralModal, { size: "large", title: this.state.Title, isOpen: this.state.IsOpenCreateOrUpdateModal, close: this.closeCreateOrUpdateModal },
                React.createElement(CategoryForm, { categoryId: this.state.SelectedIdOnRow })),
            React.createElement(GeneralModal, { size: "lg", title: this.state.Title, isOpen: this.state.IsOpenDeleteModal, close: this.closeDeleteModal },
                React.createElement("div", { className: "text-center" },
                    React.createElement("div", null, BaseComponent.DELETE_MESSAGE("danh mục")),
                    React.createElement("br", null),
                    React.createElement("button", { className: "btn btn-primary", onClick: this.onDeleteCategory.bind(this), type: "submit" }, "OK")))));
    }
}
...
function mapDispatchToProps(dispatch) {
    let pagingAction = new FetchAction.CategoriesPagingAction();
    let deleteAction = new PostAction.CategoryDeleteAction();
    return {
        loadPagingCategories: (entry) => dispatch(pagingAction.listCategoriesWithPaging(entry)),
        deleteCategory: (entry) => dispatch(deleteAction.deleteCategory(entry))
    };
}
const connectedCategories = connect(mapStateToProps, mapDispatchToProps)(Categories);
export { connectedCategories as Categories };
//# sourceMappingURL=categories-paging.js.map